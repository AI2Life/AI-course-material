...
class VirusModel():
    ...
    def setup(self) -> None:
        ...
        for agent in self.random.choice(self.schedule, self.initial_outbreak_size):
            agent.infected = True
    ...
    def step(self) -> None:
        ...
        :return: None
        """
        if not any([agent.infected for agent in self.schedule]):
            sys.exit()
        for node in self.schedule:
            node.virus_check_timer += 1
            if node.virus_check_timer >= node.model.virus_check_frequency:
                node.virus_check_timer = 0
    ...
    def spread_virus(self) -> None:
        ...
        :return: None
        """
        for inf_node in self.random.permutation([node for node in self.schedule if
                                             node.infected]):
    ...
    def setup_spatially_clustered_network(self) -> None:
        ...
        while sum([len(node.neighbors) for node in self.schedule]) / 2 < num_links:
            from_node = self.random.choice(self.schedule)
            to_node = min([node for node in self.schedule if node != from_node and node not in from_node.neighbors],
    ...
    def run(self, n_step:int =150, n_nodes: Union[None, int] =None, verbose: str =True) -> None:
        ...
        for _tick in pbar:
            ...
            if verbose:
                ...
                pbar.set_postfix({'infected': len([node for node in self.schedule if
                                                     node.infected])})
    ...
    def update_data(self, curr: Any, infected: List, resistant: List,
        ...
        """
        self.step()
        infected.append(len([node for node in self.schedule if node.infected]))
        resistant.append(len([node for node in self.schedule if node.resistant]))
        susceptible.append(len([node for node in self.schedule
                                if not node.infected and not node.resistant]))
        tick.append(self.tick)
...
if __name__ == "__main__":
    ...
    model.visulize_run(n_nodes=100)